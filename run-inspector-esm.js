...
console.log('Firebase MCP Inspector Tool (ESM Version)');
console.log('----------------------------------------');
...
console.log(`Running npx @modelcontextprotocol/inspector ${distPath}`);

// Use process.spawn to run the inspector
const inspector = spawn('npx', ['@modelcontextprotocol/inspector', distPath], {
  stdio: 'inherit',
  env: process.env,
  shell: true  // This ensures the command is run in a shell, which helps with PATH resolution
});
